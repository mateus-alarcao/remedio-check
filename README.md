...
> **Deploy:** aplicação CLI executada via Docker ou localmente.  
...
O projeto utiliza **Supabase (PostgreSQL)** como banco de dados na nuvem. A tabela principal é criada via `setup.sql`:
...
A camada de acesso (`src/db.py`) suporta operações de leitura, inserção, atualização e deleção de registros.
...
- **Deploy:** Aplicação CLI — execute via Docker conforme instruções acima
